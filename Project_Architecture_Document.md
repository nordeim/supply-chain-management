# Supply Chain Management — Master Project Architecture Document (PAD) v1.8
...
**Last Updated:** 2026-09-21 (Session 11 — verification iteration: live-reference parity re-audit, E2E webServer env hardening, doc-count alignment)
**Audience:** Senior Engineers, Tech Leads, DevOps, and Onboarding Engineers
**Rule:** Every architectural decision in this document traces to a specific rationale. Nothing is here "because it's popular."

---

#### Revision Block — v1.8 (Tracked Changes)

- `[TST]` **E2E webServer hardened against a leaked DATABASE_URL.** The Playwright `webServer` inherits the shell environment, and a workspace-level `.env` can leak a stale absolute `DATABASE_URL` pointing at a missing file (the sandbox-env class documented in AGENTS.md) — `next start` then dies at boot with `PrismaClientInitializationError` and every spec fails at the 90s webServer timeout. The config now prefixes the boot command with `env -u DATABASE_URL` **only when the inherited value is an absolute `file:` URL pointing at a missing file**, so the repo `.env` (anchored by `src/lib/db-path.ts` at `<repo>/db/custom.db`) governs. An exported URL that exists, a relative one, or a non-`file:` (PostgreSQL) URL still wins — the standard precedence rule for intentional overrides is preserved, and CI (job-level `DATABASE_URL`, always valid) is unaffected. Verified red→green: total boot failure → 38/38 chromium.
- `[VER]` **Parity re-verified against the live reference (Session 11 audit).** A fresh signed-in audit (operator account, desktop 1440 + mobile 390×844) re-confirmed the Session 10 remediation holds: 21/21 value checks identical on both sides (dashboard KPIs incl. $202,610 / −1 / 11 / 6; AI suggestions CAM-002-first at $71,000 with "Suggested: 2026-07-06"; procurement 2AF134-first with the fixed 07.12.26 date; suppliers Nordic→Atlas→Pacific→Electronics with contacts and "1 products"; market trends Office-first at +1.4%; products row order LENS-003→LENS-001→LENS-002→CAM-003→CAM-002→CAM-001) and the mobile nav geometrically exact (pill 327×42 / 100px radius / 10%-white frost + blur(20px); active 129×48 white; five 34px circles; 6 items; no overflow). Audit methodology note recorded in `docs/session_16.md`: the reference serves cold deep-links a Base44 "Pages" shell — real content requires client-side (in-app) navigation.
- `[DOC]` **Doc-count alignment:** AGENTS.md ("103 tests"), CLAUDE.md ("(85 tests)" in the command table, "(Vitest, 103)" in the pyramid — three mutually contradictory counts) and PAD §7.2 ("31 specs") all corrected to the actual suite sizes (111 unit / 38 E2E).
